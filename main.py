...
def _build_nlg(
    ...
    # Clean up user_name
    display_name = user_name.strip() if (user_name and user_name.strip()) else "there"
    ...
    # Resolve drivers, ensuring top1 and top2 are always valid and distinct
    if not drivers:
        top1, top2 = "Price", "Category"
    elif len(drivers) == 1:
        top1 = drivers[0]
        top2 = "Category" if top1 != "Category" else "Price"
    else:
        top1 = drivers[0]
        top2 = drivers[1]
    ...
    def friendly(feature: str) -> str:
        ...
        if feature == "Category":
            cat = req.category if req.category else "General"
            if has_history:
                return (f"your past purchase history in our "
                        f"<strong>{cat}</strong> category")
            return f"your browsing interest in <strong>{cat}</strong> items"
        elif feature == "Price":
            # Handle non-finite price values gracefully
            if req.price is None or math.isnan(req.price) or math.isinf(req.price):
                return "its pricing aligning with your preferences"
            
            if req.price < 0:
                formatted = f"-${abs(req.price):,.2f}"
                return f"its outstanding value — priced at just <strong>{formatted}</strong>"
            
            formatted = f"${req.price:,.2f}"
            if req.price < 30:
                return f"its outstanding value — priced at just <strong>{formatted}</strong>"
            elif req.price > 500:
                return f"its premium quality positioning (<strong>{formatted}</strong>)"
            else:
                return f"its price of <strong>{formatted}</strong> matching your budget"
        return "your overall browsing preferences"

    # Intro line
    cat = req.category if req.category else "General"
    if has_history:
        intro = (f"Hi <strong>{display_name}</strong>! Based on your past purchases "
                 f"and browsing history, here is why we recommended this product.")
    else:
        intro = (f"Hi <strong>{display_name}</strong>! Based on your interest in "
                 f"<strong>{cat}</strong> items, here is why we think "
                 f"you'll love this.")
