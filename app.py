...
def show_navbar():
    ...
    with nav_left:
        st.markdown(
            "<div style='font-size:22px;font-weight:bold;padding:8px 0;'>🧠 XAI Store</div>",
            unsafe_allow_html=True
        )
    with nav_mid:
        mode_label = "Experimental" if st.session_state.xai_enabled else "Control"
        mode_color = "#22c55e" if st.session_state.xai_enabled else "#94a3b8"
        st.markdown(
            f"<div style='text-align:center;padding:6px 0;'>"
            f"<span style='font-size:13px;font-weight:600;color:{mode_color};letter-spacing:0.5px;'>"
            f"Mode: {mode_label}</span></div>",
            unsafe_allow_html=True
        )
        xai_toggle = st.toggle("Enable XAI Explanations", value=st.session_state.xai_enabled, key="xai_toggle")
        if xai_toggle != st.session_state.xai_enabled:
            st.session_state.xai_enabled = xai_toggle
            st.rerun()
    with nav_right:
        st.markdown(
            "<div style='text-align:right;font-size:18px;padding:8px 0;'>👤 Profile &nbsp;&nbsp; 🛒 Cart</div>",
            unsafe_allow_html=True
        )
    st.markdown(
        "<hr style='margin:0 0 30px 0;border:none;border-top:1px solid #1e293b;'>",
        unsafe_allow_html=True
    )
