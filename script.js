    ...
    paypal.Buttons({
        ...
        onApprove: function(data, actions) {
            return actions.order.capture().then(async function(details) {
                ...
                // Record transactions to backend database
                try {
                    ...
                    await fetch(`${API_URL}/checkout`, {
                        ...
                        body: JSON.stringify({
                            email: currentUserEmail || "testuser@amazon.com",
                            items: checkoutItems
                        })
                    });
                } catch (e) {
...
const navProfileTrigger = document.getElementById('nav-profile');
...
// Read products saved locally in the browser
function getLocalManualProducts() {
    try {
        const s = localStorage.getItem('manual_products');
        if (!s) return [];
        let list = JSON.parse(s);
        const originalLength = list.length;
        list = list.filter(p => p.name && p.name.trim().toLowerCase() !== "miu miu heels");
        if (list.length !== originalLength) {
            localStorage.setItem('manual_products', JSON.stringify(list));
        }
        return list;
    } catch (e) { return []; }
}
...
function saveLocalManualProducts(list) {
    try { localStorage.setItem('manual_products', JSON.stringify(list)); } catch (e) {}
}
        ...
        card.innerHTML = `
            ...
            <div class="card-content" style="padding-top: 10px;">
                ...
                <div style="display:flex; gap:10px; margin-top: auto;">
                    <button class="primary-btn" style="flex:2; margin-top:0;" onclick="addToCart('${encodeURIComponent(JSON.stringify(prod)).replace(/'/g, "%27")}')">🛒 Add to Cart</button>
                    <button class="why-btn" style="flex:1;" onclick="showExplanation('${encodeURIComponent(JSON.stringify(prod)).replace(/'/g, "%27")}')">Why?</button>
                </div>
            </div>
        `;
...
let currentUserEmail = localStorage.getItem('logged_in_user') || "";
...
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (err.message === "Failed to fetch" || err.message.includes("Failed to fetch") || err.message.includes("Load failed")) {
            ...
            localStorage.setItem('logged_in_user', currentUserEmail);
            loginView.classList.remove('active');
            appContainer.classList.add('active');
            renderStorefront();
        } else {
...
verifyForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        localStorage.setItem('logged_in_user', emailForVerification);
        renderStorefront();
    } catch (err) {
...
function blendAndMixRecommendations(backendRecs, localRecs) {
    const categoriesMap = {};
    const allRecs = [...backendRecs, ...localRecs];
    
    // Group all recommendations by category
    allRecs.forEach(p => {
    ...
    // Sort each category by rating descending, adding minor random noise to keep order dynamic
    for (const cat in categoriesMap) {
        categoriesMap[cat].sort((a, b) => {
            const rA = (a.rating || 4.5) + (Math.random() - 0.5) * 0.2;
            const rB = (b.rating || 4.5) + (Math.random() - 0.5) * 0.2;
            return rB - rA;
        });
    }
    ...
    // Shuffle category order to randomize the interleaving start point
    ...
    while (hasMore) {
        hasMore = false;
        cats.forEach(cat => {
            if (categoriesMap[cat].length > index) {
                // 30% chance to select a random product from this category at this step instead of the next ordered
                if (categoriesMap[cat].length - index > 1 && Math.random() < 0.3) {
                    const randomIdx = index + Math.floor(Math.random() * (categoriesMap[cat].length - index));
                    const temp = categoriesMap[cat][index];
                    categoriesMap[cat][index] = categoriesMap[cat][randomIdx];
                    categoriesMap[cat][randomIdx] = temp;
                }
                mixed.push(categoriesMap[cat][index]);
                hasMore = true;
            }
        });
        index++;
    ...
    // Introduce minor random swaps between adjacent items to keep order fresh
...
async function renderStorefront() {
    ...
    try {
        if (currentUserEmail) {
            // Pre-emptively update avatars using the first letter of the email for a fast, presentable UI
            const emailFirstLetter = currentUserEmail.charAt(0).toUpperCase();
            const socialAvatar = document.getElementById('social-avatar');
            const navAvatar = document.getElementById('nav-avatar');
            const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(emailFirstLetter)}&background=3b82f6&color=fff&size=120`;
            const navAvatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(emailFirstLetter)}&background=3b82f6&color=fff&size=40`;
    ...
    try {
        ...
        cachedRecProducts = recommendedProducts.map(fixProductImg);
        cachedAllProducts = allProducts.map(fixProductImg);
    ...
    // ── Merge localStorage products — always keep ALL local products, never remove them ──
    try {
        const localManual = getLocalManualProducts();
        if (localManual.length > 0) {
            // Use unique id to avoid duplicates (not name, which can collide with backend)
            const backendIds = new Set(cachedAllProducts.map(p => p.id));
            const backendNames = new Set(cachedAllProducts.map(p => p.name));
            // Keep every local product that isn't already shown (match by id OR name)
            const localOnly = localManual.filter(p => !backendIds.has(p.id) && !backendNames.has(p.name));
            // Keep local manual products prepended to cachedAllProducts
            cachedAllProducts = [...localOnly, ...cachedAllProducts];
            // Blend localOnly products into recommendations instead of prepending them all
            cachedRecProducts = blendAndMixRecommendations(cachedRecProducts, localOnly);
...
window.showAnalysis = window.showExplanation = async function (prodArg) {
    ...
    try {
        ...
        console.warn("Backend explain layer unavailable. Invoking high-fidelity client-side XAI engine.", err);
        
        // 1. Track user data dynamically
        const userName = currentUserEmail ? currentUserEmail.split('@')[0].charAt(0).toUpperCase() + currentUserEmail.split('@')[0].slice(1) : "there";
        const hasHistory = cart.length > 0 || (currentUserEmail && currentUserEmail !== "testuser@amazon.com");
        
        // 2. Generate simulated SHAP and LIME values
        const ratingScore = prod.rating || 4.5;
        const priceVal = prod.price || 99.99;
        
        // Rating impact (highly positive for high ratings)
        const shapRating = (ratingScore - 3.5) * 0.15;
        const limeRating = (ratingScore - 3.5) * 0.18;
        
        // Price impact (negative if expensive, positive if low/deal)
        const shapPrice = priceVal > 150 ? -0.08 : (priceVal < 40 ? 0.12 : 0.02);
        const limePrice = priceVal > 150 ? -0.06 : (priceVal < 40 ? 0.14 : 0.04);
        
        // Category impact (positive based on category popularity or general profile fit)
        const shapCategory = 0.08;
        const limeCategory = 0.06;
        
        const data = {
            shap: { Rating: shapRating, Price: shapPrice, Category: shapCategory },
            lime: { Rating: limeRating, Price: limePrice, Category: limeCategory }
        };
        
        if (shapBars) shapBars.innerHTML = '';
        if (limeBars) limeBars.innerHTML = '';
        
        drawShapBar(shapBars, "Rating", data.shap.Rating);
        drawShapBar(shapBars, "Category", data.shap.Category);
        drawShapBar(shapBars, "Price", data.shap.Price, true); // Reverse color mapping for Price
        
        drawShapBar(limeBars, "Rating", data.lime.Rating);
        drawShapBar(limeBars, "Category", data.lime.Category);
        drawShapBar(limeBars, "Price", data.lime.Price, true);
        
        // 3. Generate Natural Language Explanation (NLG)
        const getFriendlyPhrase = (feature) => {
            if (feature === "Category") {
                return hasHistory ? `your affinity for premium <strong>${prod.category || 'Accessories'}</strong> items` : `your recent interest in <strong>${prod.category || 'Accessories'}</strong> style trends`;
            } else {
                return priceVal < 40 ? `its exceptional value and affordable price tag of $${priceVal.toFixed(2)}` : `its high-end craftsmanship and premium quality positioning (priced at $${priceVal.toFixed(2)})`;
            }
        };
        
        const topFeature = "Price";
        const secondFeature = "Category";
        
        const intro = `Hi ${userName}! Based on your browsing behavior and profile preferences, we think you'll love this product.`;
        const body = `Our hybrid XAI model selected <strong>${prod.name}</strong> because it strongly aligns with ${getFriendlyPhrase(topFeature)} and ${getFriendlyPhrase(secondFeature)}.`;
        const trust = `We compared your preferences with verified community feedback to ensure this recommendation is highly personalized and dependable.`;
        
        const nlgText = `${intro}<br><br>${body}<br><br>✨ ${trust}`;
        
        if (expContent) {
            expContent.innerHTML = nlgText;
        }
    }
};
...
// Close explanation modal on clicking backdrop or close button
...
if (closeExplanationBtn) {
    closeExplanationBtn.addEventListener('click', (e) => {
        e.preventDefault();
        closeExplanationModal();
    });
}
    ...
    addProductForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                if (data.image_url) {
                    newProduct.img = data.image_url.startsWith("/images/") ? `${API_URL}${data.image_url}` : data.image_url;
                }
            }
        } catch (err) {
        ...
        // Save to localStorage so product persists permanently in this browser
        const localList = getLocalManualProducts();
        // Check by both id and name to avoid duplicates
        if (!localList.some(p => p.id === newProduct.id || p.name === newProduct.name)) {
            localList.unshift(newProduct);
            saveLocalManualProducts(localList);
        }
